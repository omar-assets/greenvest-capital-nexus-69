...
serve(async (req) => {
  ...
  try {
    ...
    if (!company_id || !external_app_id) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: company_id and external_app_id' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    console.log(`Generating scorecard for app_id: ${external_app_id}, company_id: ${company_id}`)
